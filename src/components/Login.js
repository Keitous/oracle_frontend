...
const Login = () => {
    ...
    useEffect(() => {
        const checkFirstLogin = async () => {
            try {
                if (login) {
                    const response = await axios.get(`https://getv.serveo.net/api/check-first-login/${login}`);
    ...
    const handleSubmit = async (event) => {
        ...
        if (isFirstLogin) {
            ...
            try {
                const response = await axios.post('https://getv.serveo.net/api/login', {
                    login,
                    defaultPassword,
                    newPassword
                });
                ...
                if (response.data.success) {
                    ...
                    const updateResponse = await axios.post('https://getv.serveo.net/api/update-password', {
                        login,
                        newPassword
                    });
    ...
    const fetchUserDetails = async () => {
        try {
            const response = await axios.get(`https://getv.serveo.net/api/users/${login}`);
